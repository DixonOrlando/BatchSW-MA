...
BatchedPowerSim_conts <- function(DesMatrix, Kseq, m, Batch, Teffs, Rxeff, ICC, CAC, corrstruct, treateffvar){
  ...
  if (CAC == 1){ #If satisfied, then fit exchangeable model
    ...
    #Number of time individual linear-mixed models inside the meta-analysis procedure managed to converge. Suppose batch = 5, if 4 out of 5 models ran successfully
    #then number_success_meta = 4 instead of 5.
...
BatchedPowerSim_conts_wrap <- 
  function(nrep, Ts, nbatch, olap, Kseq, m, TimeEffsInd, Rxeff, ICC, CAC, corrstruct, treateffvar){
    ...
    #Repiclating the simulation nrep number of times.
    ...
    #Now convert it into numeric, so numerical calculation can be performed
    ...
    #- Average Model SE
    #- Coverage
    #- nrep_conv: number of replication with succesful convergence; this variable has different definition for linear-mixed model approach and the meta-analysis approach (check the methods section in the writing)
    #- numb_success: this variable is constructed for the meta-analysis approach in mind; it calculates the number of times the individual linear-mixed models managed to converge inside the meta-analysis approach across all replications
    #- numb_warnings: number of warnings across nrep-replications 
    #- numb_errors: number of errors across nrep-replications
    
    #NOTE: the value for nrep_conv and numb_success variables for linear-mixed models will be identical, but will be different for the meta-analysis models. 
    #For the meta-analysis procedure, the maximum value for numb_success variable will be the number of replications times the number of batches.
    #For example, given 500 replications and 5 batches, the maximum value for numb_success variable will be 2,500.
    #However, if among the 2,500 models, 100 failed to converge, then the final value for numb_success variable will be 2,400.
    #This variable will let us know in what situation the individual linear-mixed models inside the meta-analysis approach failed to converge the most. 
    #The nrep_conv variable for the meta-analysis approach will be the number of replications where at least one of the individual linear-mixed models managed to converge.
    #For example, in a replication with 5 batches, if 1-5 individual linear-mixed models managed to converge then this will be regarded as convergence in the nrep_conv variable.
...
#This is the list of parameters considered for the simulation study.
Tlist <- c(4, 6) 
nbatchlist <- c(2, 5) 
...
#Calculating T-2 in the olap variable.
...
#Loop to perform simulation study.
for (i in 1:nrow(allcombos)) {
  nrep = 500 #Performing 500 replications for each scenario.
  if (i == 1) {
