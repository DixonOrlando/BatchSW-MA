###Analysing the results of the simulation

library(tidyverse)
library(looplot)
library(RColorBrewer)
library(dplyr)
library(ggpubr)

#Reading the results from the simulation study and preparing it for analysis.
setwd("C:/Users/dorl0004/OneDrive/Monash Biostatistics/GitHub code")
all_contsresults = read_csv("all_contsresults_BE&E_final.csv")

all_output = read_csv("all_output_BE&E_final.csv")

all_contsresults = all_contsresults %>%
  dplyr::select(-`...1`)

MA_checkdata = read_csv("MA_check_finaldata.csv") %>%
  dplyr::select(-`...1`)

#Changing the values of both nolap and nbatch, so it's easier to read in the plots.

all_contsresults = all_contsresults %>%
  mutate(nolap = as.character(nolap),
    nolap = case_when(
      nolap == "1" ~ "1 period of overlap",
    nolap == "2" | nolap == "4" ~ "T-2 periods of overlap"
  ),
  nolap = factor(nolap, levels = c("1 period of overlap", "T-2 periods of overlap")),
  nbatch = as.character(nbatch),
  nbatch = case_when(
    nbatch == "2" ~ "2 batches",
    nbatch == "5" ~ "5 batches"
  ),
  nbatch = factor(nbatch, levels = c("2 batches", "5 batches"))
  )

all_contsresults = all_contsresults %>%
  left_join(MA_checkdata, by = c("model" = "Model", "comb_ID" = "comb_ID"))

all_output = all_output %>%
  mutate(nolap = as.character(nolap),
         nolap = case_when(
           nolap == "1" ~ "1 period of overlap",
           nolap == "2" | nolap == "4" ~ "T-2 periods of overlap"
         ),
         nolap = factor(nolap, levels = c("1 period of overlap", "T-2 periods of overlap")),
         nbatch = as.character(nbatch),
         nbatch = case_when(
           nbatch == "2" ~ "2 batches",
           nbatch == "5" ~ "5 batches"
         ),
         nbatch = factor(nbatch, levels = c("2 batches", "5 batches"))
  )

#Transforming into appropriate format for each sections.

all_contsresults_bias = all_contsresults %>%
  dplyr::select(model, Bias, Bias_minusSE, Bias_plusSE, T, nbatch, nolap, K, M, sharedtime, effsize, ICC, CAC, corrstruct, treateffvar, comb_ID) %>%
  mutate(with_corr = case_when(
    CAC == 1 ~ "Exch",
    CAC != 1 & corrstruct == 0 ~ "BE",
    CAC != 1 & corrstruct == 1 ~ "DTD"
  )) %>%
  pivot_wider(names_from = model,
              values_from = c(Bias, Bias_minusSE, Bias_plusSE)) 

all_contsresults_EmpSE = all_contsresults %>%
  dplyr::select(model, Emp_SE, EmpSE_minusSE, EmpSE_plusSE, T, nbatch, nolap, K, M, sharedtime, effsize, ICC, CAC, corrstruct, treateffvar, comb_ID) %>%
  mutate(with_corr = case_when(
    CAC == 1 ~ "Exch",
    CAC != 1 & corrstruct == 0 ~ "BE",
    CAC != 1 & corrstruct == 1 ~ "DTD"
  )) %>%
  pivot_wider(names_from = model,
              values_from = c(Emp_SE, EmpSE_minusSE, EmpSE_plusSE))

all_contsresults_coverage = all_contsresults %>%
  dplyr::select(model, Coverage, Coverage_minusSE, Coverage_plusSE, T, nbatch, nolap, K, M, sharedtime, effsize, ICC, CAC, corrstruct, treateffvar, comb_ID) %>%
  mutate(with_corr = case_when(
    CAC == 1 ~ "Exch",
    CAC != 1 & corrstruct == 0 ~ "BE",
    CAC != 1 & corrstruct == 1 ~ "DTD"
  )) %>%
  pivot_wider(names_from = model,
              values_from = c(Coverage, Coverage_minusSE, Coverage_plusSE))

all_contsresults_nonconv = all_contsresults %>%
  dplyr::mutate(nonconv = 1 - (nrep_conv.x/500)
  ) %>%
  dplyr::select(model, nonconv, T, nbatch, nolap, K, M, sharedtime, effsize, ICC, CAC, corrstruct, treateffvar, comb_ID) %>%
  mutate(with_corr = case_when(
    CAC == 1 ~ "Exch",
    CAC != 1 & corrstruct == 0 ~ "BE",
    CAC != 1 & corrstruct == 1 ~ "DTD"
  )) %>%
  tidyr::pivot_wider(names_from = model,
                     values_from = nonconv)

all_contsresults_MACHECK = all_contsresults %>%
  dplyr::select(model, final_conditional, T, nbatch, nolap, K, M, sharedtime, effsize, ICC, CAC, corrstruct, treateffvar, comb_ID) %>%
  mutate(with_corr = case_when(
    CAC == 1 ~ "Exch",
    CAC != 1 & corrstruct == 0 ~ "BE",
    CAC != 1 & corrstruct == 1 ~ "DTD"
  )) %>%
  filter(!grepl("mod_A|mod_B|mod_C|mod_D", model)) %>%
  tidyr::pivot_wider(names_from = model,
                     values_from = final_conditional)

########################Setting the color, so they are consistent########################
c_modA = brewer.pal(8, name = "Dark2")[1]
c_modB = brewer.pal(8, name = "Dark2")[2]
c_modC = brewer.pal(8, name = "Dark2")[3]
c_modD = brewer.pal(8, name = "Dark2")[4]

c_mafixed = brewer.pal(8, name = "Dark2")[5]
c_maclassic = brewer.pal(8, name = "Dark2")[6]
c_maHK = brewer.pal(8, name = "Dark2")[7]
c_maHK_seadhoc = brewer.pal(8, name = "Dark2")[8]
c_maHK_iqadhoc = "#FFC107"
c_maKR = "#00BCD4"




########################All the meta-analysis plots first########################
setwd("C:/Users/dorl0004/OneDrive/Monash Biostatistics/GitHub code/Revised plots")

#Prespecifying the steps for the nested loop plots.
steps = c("K",  "T", "ICC", "CAC", "treateffvar") 
steps_names = c("Clusters (K)", "Periods (T)", "ICC", "CAC", expression(sigma[eta]^2))

######Bias plot######

bias_MA_separate_SJ = nested_loop_plot(resdf = all_contsresults_bias %>%
                                         filter(sharedtime == 1 & effsize == 0) %>%
                                         mutate(M = factor(M)),
                                       grid_labeller = labeller(nbatch = label_value, nolap = label_value),
                                       x = "M", 
                                       x_name = "Number of participants per cluster-period (m)",
                                       methods = c("Bias_MA.random.SJ.classic", "Bias_MA.random.SJ.HK", "Bias_MA.random.SJ.HK.SEadhoc", "Bias_MA.random.SJ.HK.IQadhoc", "Bias_MA.random.SJ.KR"),
                                       steps = steps,
                                       colors = c(c_maclassic, c_maHK, c_maHK_seadhoc, c_maHK_iqadhoc, c_maKR),
                                       steps_color = "#000000",
                                       steps_annotation_color = "#000000",
                                       point_shapes = c(17, 17, 17, 17, 17),
                                       point_size = 1.2,
                                       steps_annotation_size = 3.5,
                                       steps_y_base = -0.017,
                                       steps_y_height = 0.005,
                                       steps_y_shift = 0.005, #Separate the distance between step
                                       steps_annotation_nudge = 0.1,
                                       steps_values_annotate = T,
                                       steps_names = steps_names,
                                       grid_rows = "nbatch",
                                       grid_cols = "nolap",
                                       y_name = "Bias",
                                       hline_intercept = 0,
                                       spu_x_shift = 2,
                                       legend_name = "Model",
                                       legend_labels = c("Random MA; Tau:SJ; CI:classic", "Random MA; Tau:SJ; CI:HK", "Random MA; Tau:SJ; CI:HK.SEadhoc", "Random MA; Tau:SJ; CI:HK.IQadhoc", "Random MA; Tau:SJ; CI:KR"),
                                       post_processing = list(
                                         add_custom_theme = list(
                                           legend.position="bottom",
                                           axis.text.x = element_text(angle = -90, 
                                                                      vjust = 0.5, 
                                                                      size = 5) 
                                         )))

bias_MA_shared_SJ = nested_loop_plot(resdf = all_contsresults_bias %>%
                                       filter(sharedtime != 1 & effsize == 0) %>%
                                       mutate(M = factor(M)),
                                     grid_labeller = labeller(nbatch = label_value, nolap = label_value),
                                     x = "M", 
                                     x_name = "Number of participants per cluster-period (m)",
                                       methods = c("Bias_MA.random.SJ.classic", "Bias_MA.random.SJ.HK", "Bias_MA.random.SJ.HK.SEadhoc", "Bias_MA.random.SJ.HK.IQadhoc", "Bias_MA.random.SJ.KR"),
                                       steps = steps,
                                     colors = c(c_maclassic, c_maHK, c_maHK_seadhoc, c_maHK_iqadhoc, c_maKR),
                                     steps_color = "#000000",
                                     steps_annotation_color = "#000000",
                                     point_shapes = c(17, 17, 17, 17, 17),
                                     point_size = 1.2,
                                     steps_annotation_size = 3.5,
                                       steps_y_base = -0.03,
                                       steps_y_height = 0.005,
                                       steps_y_shift = 0.005, #Separate the distance between step
                                       steps_annotation_nudge = 0.1,
                                       steps_values_annotate = T,
                                       steps_names = steps_names,
                                       grid_rows = "nbatch",
                                       grid_cols = "nolap",
                                       y_name = "Bias",
                                      hline_intercept = 0,
                                     spu_x_shift = 2,
                                       legend_name = "Model",
                                       legend_labels = c("Random MA; Tau:SJ; CI:classic", "Random MA; Tau:SJ; CI:HK", "Random MA; Tau:SJ; CI:HK.SEadhoc", "Random MA; Tau:SJ; CI:HK.IQadhoc", "Random MA; Tau:SJ; CI:KR"),
                                       post_processing = list(
                                         add_custom_theme = list(
                                           legend.position="bottom",
                                           axis.text.x = element_text(angle = -90, 
                                                                      vjust = 0.5, 
                                                                      size = 5) 
                                         )))
#Saving the results.
bias_MA_separate_SJ
ggsave("bias_MA_separate_SJ.png", height = 13, width = 15)
bias_MA_shared_SJ
ggsave("bias_MA_shared_SJ.png", height = 13, width = 15)

bias_MA_separate_reml = nested_loop_plot(resdf = all_contsresults_bias %>%
                                           filter(sharedtime == 1 & effsize == 0) %>%
                                           mutate(M = factor(M)),
                                         grid_labeller = labeller(nbatch = label_value, nolap = label_value),
                                         x = "M", 
                                         x_name = "Number of participants per cluster-period (m)",
                                         methods = c("Bias_MA.random.reml.classic", "Bias_MA.random.reml.HK", "Bias_MA.random.SJ.HK.SEadhoc", "Bias_MA.random.reml.HK.IQadhoc", "Bias_MA.random.reml.KR"),
                                         steps = steps,
                                         colors = c(c_maclassic, c_maHK, c_maHK_seadhoc, c_maHK_iqadhoc, c_maKR),
                                         steps_color = "#000000",
                                         steps_annotation_color = "#000000",
                                         point_size = 1.2,
                                         steps_annotation_size = 3.5,
                                         point_shapes = c(17, 17, 17, 17, 17),
                                         steps_y_base = -0.017,
                                         steps_y_height = 0.005,
                                         steps_y_shift = 0.005, #Separate the distance between step
                                         steps_annotation_nudge = 0.1,
                                         steps_values_annotate = T,
                                         steps_names = steps_names,
                                         grid_rows = "nbatch",
                                         grid_cols = "nolap",
                                         y_name = "Bias",
                                         hline_intercept = 0,
                                         legend_name = "Model",
                                         spu_x_shift = 2,
                                         legend_labels = c("Random MA; Tau:REML; CI:classic", "Random MA; Tau:REML; CI:HK", "Random MA; Tau:REML; CI:HK.SEadhoc", "Random MA; Tau:REML; CI:HK.IQadhoc", "Random MA; Tau:REML; CI:KR"),
                                         post_processing = list(
                                           add_custom_theme = list(
                                             legend.position="bottom",
                                             axis.text.x = element_text(angle = -90, 
                                                                        vjust = 0.5, 
                                                                        size = 5) 
                                           )))

bias_MA_shared_reml = nested_loop_plot(resdf = all_contsresults_bias %>%
                                         filter(sharedtime != 1 & effsize == 0) %>%
                                         mutate(M = factor(M)),
                                       grid_labeller = labeller(nbatch = label_value, nolap = label_value),
                                       x = "M", 
                                       x_name = "Number of participants per cluster-period (m)",
                                         methods = c("Bias_MA.random.reml.classic", "Bias_MA.random.reml.HK", "Bias_MA.random.SJ.HK.SEadhoc", "Bias_MA.random.reml.HK.IQadhoc", "Bias_MA.random.reml.KR"),
                                         steps = steps,
                                       colors = c(c_maclassic, c_maHK, c_maHK_seadhoc, c_maHK_iqadhoc, c_maKR),
                                       steps_color = "#000000",
                                       steps_annotation_color = "#000000",
                                       point_shapes = c(17, 17, 17, 17, 17),
                                       point_size = 1.2,
                                       steps_annotation_size = 3.5,
                                         steps_y_base = -0.017,
                                         steps_y_height = 0.005,
                                         steps_y_shift = 0.005, #Separate the distance between step
                                         steps_annotation_nudge = 0.1,
                                         steps_values_annotate = T,
                                       spu_x_shift = 2,
                                         steps_names = steps_names,
                                         grid_rows = "nbatch",
                                         grid_cols = "nolap",
                                         y_name = "Bias",
                                         hline_intercept = 0,
                                         legend_name = "Model",
                                         legend_labels = c("Random MA; Tau:REML; CI:classic", "Random MA; Tau:REML; CI:HK", "Random MA; Tau:REML; CI:HK.SEadhoc", "Random MA; Tau:REML; CI:HK.IQadhoc", "Random MA; Tau:REML; CI:KR"),
                                         post_processing = list(
                                           add_custom_theme = list(
                                             legend.position="bottom",
                                             axis.text.x = element_text(angle = -90, 
                                                                        vjust = 0.5, 
                                                                        size = 5) 
                                           )))
#Saving the results
bias_MA_separate_reml
ggsave("bias_MA_separate_REML.png", height = 13, width = 15)
bias_MA_shared_reml
ggsave("bias_MA_shared_REML.png", height = 13, width = 15)

######Coverage######

coverage_MA_separate_SJ = nested_loop_plot(resdf = all_contsresults_coverage %>%
                                             filter(sharedtime == 1 & effsize == 0) %>%
                                             mutate(M = factor(M)),
                                           grid_labeller = labeller(nbatch = label_value, nolap = label_value),
                                           x = "M", 
                                           x_name = "Number of participants per cluster-period (m)",
                                       methods = c("Coverage_MA.random.SJ.classic", "Coverage_MA.random.SJ.HK", "Coverage_MA.random.SJ.HK.SEadhoc", "Coverage_MA.random.SJ.HK.IQadhoc", "Coverage_MA.random.SJ.KR"),
                                       steps = steps,
                                       colors = c(c_maclassic, c_maHK, c_maHK_seadhoc, c_maHK_iqadhoc, c_maKR),
                                       steps_color = "#000000",
                                       steps_annotation_color = "#000000",
                                       point_size = 1.2,
                                       steps_annotation_size = 3.5,
                                       point_shapes = c(17, 17, 17, 17, 17),
                                      steps_y_base = 0.65,
                                       steps_y_height = 0.04,
                                       steps_y_shift = 0.04,
                                       spu_x_shift = 2,
                                       steps_annotation_nudge = 0.1,
                                       steps_values_annotate = T,
                                       steps_names = steps_names,
                                       grid_rows = "nbatch",
                                       grid_cols = "nolap",
                                       y_name = "Coverage",
                                       hline_intercept = 0.95,
                                       legend_name = "Model",
                                       legend_labels = c("Random MA; Tau:SJ; CI:classic", "Random MA; Tau:SJ; CI:HK", "Random MA; Tau:SJ; CI:HK.SEadhoc", "Random MA; Tau:SJ; CI:HK.IQadhoc", "Random MA; Tau:SJ; CI:KR"),
                                       point_alpha = 0.3,
                                       post_processing = list(
                                         add_custom_theme = list(
                                           legend.position="bottom",
                                           axis.text.x = element_text(angle = -90, 
                                                                      vjust = 0.5, 
                                                                      size = 5) 
                                         )))

coverage_MA_shared_SJ = nested_loop_plot(resdf = all_contsresults_coverage %>%
                                           filter(sharedtime != 1 & effsize == 0) %>%
                                           mutate(M = factor(M)),
                                         grid_labeller = labeller(nbatch = label_value, nolap = label_value),
                                         x = "M", 
                                         x_name = "Number of participants per cluster-period (m)",
                                     methods = c("Coverage_MA.random.SJ.classic", "Coverage_MA.random.SJ.HK", "Coverage_MA.random.SJ.HK.SEadhoc", "Coverage_MA.random.SJ.HK.IQadhoc", "Coverage_MA.random.SJ.KR"),
                                     steps = steps,
                                     colors = c(c_maclassic, c_maHK, c_maHK_seadhoc, c_maHK_iqadhoc, c_maKR),
                                     steps_color = "#000000",
                                     steps_annotation_color = "#000000",
                                     point_shapes = c(17, 17, 17, 17, 17),
                                     point_size = 1.2,
                                     steps_annotation_size = 3.5,
                                     steps_y_base = 0.65,
                                     steps_y_height = 0.04,
                                     steps_y_shift = 0.04,
                                     spu_x_shift = 2,
                                     steps_annotation_nudge = 0.1,
                                     steps_values_annotate = T,
                                     steps_names = steps_names,
                                     grid_rows = "nbatch",
                                     grid_cols = "nolap",
                                     y_name = "Coverage",
                                     hline_intercept = 0.95,
                                     legend_name = "Model",
                                     legend_labels = c("Random MA; Tau:SJ; CI:classic", "Random MA; Tau:SJ; CI:HK", "Random MA; Tau:SJ; CI:HK.SEadhoc", "Random MA; Tau:SJ; CI:HK.IQadhoc", "Random MA; Tau:SJ; CI:KR"),
                                     point_alpha = 0.3,
                                     post_processing = list(
                                       add_custom_theme = list(
                                         legend.position="bottom",
                                         axis.text.x = element_text(angle = -90, 
                                                                    vjust = 0.5, 
                                                                    size = 5) 
                                       )))

#Saving the results
coverage_MA_separate_SJ
ggsave("coverage_MA_separate_SJ.png", height = 13, width = 15)
coverage_MA_shared_SJ
ggsave("coverage_MA_shared_SJ.png", height = 13, width = 15)

coverage_MA_separate_reml = nested_loop_plot(resdf = all_contsresults_coverage %>%
                                               filter(sharedtime == 1 & effsize == 0) %>%
                                               mutate(M = factor(M)),
                                             grid_labeller = labeller(nbatch = label_value, nolap = label_value),
                                             x = "M", 
                                             x_name = "Number of participants per cluster-period (m)",
                                         methods = c("Coverage_MA.random.reml.classic", "Coverage_MA.random.reml.HK", "Coverage_MA.random.reml.HK.SEadhoc", "Coverage_MA.random.reml.HK.IQadhoc", "Coverage_MA.random.reml.KR"),
                                         steps = steps,
                                         colors = c(c_maclassic, c_maHK, c_maHK_seadhoc, c_maHK_iqadhoc, c_maKR),
                                         steps_color = "#000000",
                                         steps_annotation_color = "#000000",
                                         point_shapes = c(17, 17, 17, 17, 17),
                                         point_size = 1.2,
                                         steps_annotation_size = 3.5,
                                         steps_y_base = 0.65,
                                         steps_y_height = 0.04,
                                         steps_y_shift = 0.04,
                                         spu_x_shift = 2,
                                         steps_annotation_nudge = 0.1,
                                         steps_values_annotate = T,
                                         steps_names = steps_names,
                                         grid_rows = "nbatch",
                                         grid_cols = "nolap",
                                         y_name = "Coverage",
                                         hline_intercept = 0.95,
                                         legend_name = "Model",
                                         legend_labels = c("Random MA; Tau:REML; CI:classic", "Random MA; Tau:REML; CI:HK", "Random MA; Tau:REML; CI:HK.SEadhoc", "Random MA; Tau:REML; CI:HK.IQadhoc", "Random MA; Tau:REML; CI:KR"),
                                         point_alpha = 0.3,
                                         post_processing = list(
                                           add_custom_theme = list(
                                             legend.position="bottom",
                                             axis.text.x = element_text(angle = -90, 
                                                                        vjust = 0.5, 
                                                                        size = 5) 
                                           )))

coverage_MA_shared_reml = nested_loop_plot(resdf = all_contsresults_coverage %>%
                                             filter(sharedtime != 1 & effsize == 0) %>%
                                             mutate(M = factor(M)),
                                           grid_labeller = labeller(nbatch = label_value, nolap = label_value),
                                           x = "M", 
                                           x_name = "Number of participants per cluster-period (m)",
                                       methods = c("Coverage_MA.random.reml.classic", "Coverage_MA.random.reml.HK", "Coverage_MA.random.reml.HK.SEadhoc", "Coverage_MA.random.reml.HK.IQadhoc", "Coverage_MA.random.reml.KR"),
                                       steps = steps,
                                       colors = c(c_maclassic, c_maHK, c_maHK_seadhoc, c_maHK_iqadhoc, c_maKR),
                                       steps_color = "#000000",
                                       steps_annotation_color = "#000000",
                                       point_size = 1.2,
                                       steps_annotation_size = 3.5,
                                       point_shapes = c(17, 17, 17, 17, 17),
                                       steps_y_base = 0.65,
                                       steps_y_height = 0.04,
                                       steps_y_shift = 0.04,
                                       steps_annotation_nudge = 0.1,
                                       steps_values_annotate = T,
                                       spu_x_shift = 2,
                                       steps_names = steps_names,
                                       grid_rows = "nbatch",
                                       grid_cols = "nolap",
                                       y_name = "Coverage",
                                       hline_intercept = 0.95,
                                       legend_name = "Model",
                                       legend_labels = c("Random MA; Tau:REML; CI:classic", "Random MA; Tau:REML; CI:HK", "Random MA; Tau:REML; CI:HK.SEadhoc", "Random MA; Tau:REML; CI:HK.IQadhoc", "Random MA; Tau:REML; CI:KR"),
                                       point_alpha = 0.3,
                                       post_processing = list(
                                         add_custom_theme = list(
                                           legend.position="bottom",
                                           axis.text.x = element_text(angle = -90, 
                                                                      vjust = 0.5, 
                                                                      size = 5) 
                                         )))

#Saving the results
coverage_MA_separate_reml
ggsave("coverage_MA_separate_REML.png", height = 13, width = 15)
coverage_MA_shared_reml
ggsave("coverage_MA_shared_REML.png", height = 13, width = 15)

######Empirical standard error######

EmpSE_MA_separate_SJ = nested_loop_plot(resdf = all_contsresults_EmpSE %>%
                                          filter(sharedtime == 1 & effsize == 0) %>%
                                          mutate(M = factor(M)),
                                        grid_labeller = labeller(nbatch = label_value, nolap = label_value),
                                        x = "M", 
                                        x_name = "Number of participants per cluster-period (m)",
                                           methods = c("Emp_SE_MA.random.SJ.classic", "Emp_SE_MA.random.SJ.HK", "Emp_SE_MA.random.SJ.HK.SEadhoc", "Emp_SE_MA.random.SJ.HK.IQadhoc", "Emp_SE_MA.random.SJ.KR"),
                                           steps = steps,
                                        colors = c(c_maclassic, c_maHK, c_maHK_seadhoc, c_maHK_iqadhoc, c_maKR),
                                        steps_color = "#000000",
                                        steps_annotation_color = "#000000",
                                        point_shapes = c(17, 17, 17, 17, 17),
                                        point_size = 1.2,
                                        steps_annotation_size = 3.5,
                                           steps_y_base = -0.014,
                                        steps_y_height = 0.01,
                                        steps_y_shift = 0.03,
                                        spu_x_shift = 2,
                                           steps_annotation_nudge = 0.1,
                                           steps_values_annotate = T,
                                           steps_names = steps_names,
                                           grid_rows = "nbatch",
                                           grid_cols = "nolap",
                                           y_name = "Empirical standard error",
                                           hline_intercept = 0,
                                           legend_name = "Model",
                                           legend_labels = c("Random MA; Tau:SJ; CI:classic", "Random MA; Tau:SJ; CI:HK", "Random MA; Tau:SJ; CI:HK.SEadhoc", "Random MA; Tau:SJ; CI:HK.IQadhoc", "Random MA; Tau:SJ; CI:KR"),
                                           post_processing = list(
                                             add_custom_theme = list(
                                               legend.position="bottom",
                                               axis.text.x = element_text(angle = -90, 
                                                                          vjust = 0.5, 
                                                                          size = 5) 
                                             )))

EmpSE_MA_shared_SJ = nested_loop_plot(resdf = all_contsresults_EmpSE %>%
                                        filter(sharedtime != 1 & effsize == 0) %>%
                                        mutate(M = factor(M)),
                                      grid_labeller = labeller(nbatch = label_value, nolap = label_value),
                                      x = "M", 
                                      x_name = "Number of participants per cluster-period (m)",
                                      methods = c("Emp_SE_MA.random.SJ.classic", "Emp_SE_MA.random.SJ.HK", "Emp_SE_MA.random.SJ.HK.SEadhoc", "Emp_SE_MA.random.SJ.HK.IQadhoc", "Emp_SE_MA.random.SJ.KR"),
                                         steps = steps,
                                      colors = c(c_maclassic, c_maHK, c_maHK_seadhoc, c_maHK_iqadhoc, c_maKR),
                                      steps_color = "#000000",
                                      steps_annotation_color = "#000000",
                                      point_shapes = c(17, 17, 17, 17, 17),
                                      point_size = 1.2,
                                      steps_annotation_size = 3.5,
                                          steps_y_base = -0.014,
                                      steps_y_height = 0.01,
                                      steps_y_shift = 0.03,
                                      spu_x_shift = 2,
                                         steps_annotation_nudge = 0.1,
                                         steps_values_annotate = T,
                                         steps_names = steps_names,
                                         grid_rows = "nbatch",
                                         grid_cols = "nolap",
                                         y_name = "Empirical standard error",
                                         hline_intercept = 0,
                                         legend_name = "Model",
                                         legend_labels = c("Random MA; Tau:SJ; CI:classic", "Random MA; Tau:SJ; CI:HK", "Random MA; Tau:SJ; CI:HK.SEadhoc", "Random MA; Tau:SJ; CI:HK.IQadhoc", "Random MA; Tau:SJ; CI:KR"),
                                         post_processing = list(
                                           add_custom_theme = list(
                                             legend.position="bottom",
                                             axis.text.x = element_text(angle = -90, 
                                                                        vjust = 0.5, 
                                                                        size = 5) 
                                           )))

#Saving the results
EmpSE_MA_separate_SJ
ggsave("EmpSE_MA_separate_SJ.png", height = 13, width = 15)
EmpSE_MA_shared_SJ
ggsave("EmpSE_MA_shared_SJ.png", height = 13, width = 15)

EmpSE_MA_separate_reml = nested_loop_plot(resdf = all_contsresults_EmpSE %>%
                                            filter(sharedtime == 1 & effsize == 0) %>%
                                            mutate(M = factor(M)),
                                          grid_labeller = labeller(nbatch = label_value, nolap = label_value),
                                          x = "M", 
                                          x_name = "Number of participants per cluster-period (m)",
                                             methods = c("Emp_SE_MA.random.reml.classic", "Emp_SE_MA.random.reml.HK", "Emp_SE_MA.random.reml.HK.SEadhoc", "Emp_SE_MA.random.reml.HK.IQadhoc", "Emp_SE_MA.random.reml.KR"),
                                             steps = steps,
                                          colors = c(c_maclassic, c_maHK, c_maHK_seadhoc, c_maHK_iqadhoc, c_maKR),
                                          steps_color = "#000000",
                                          steps_annotation_color = "#000000",
                                          point_shapes = c(17, 17, 17, 17, 17),
                                          point_size = 1.2,
                                          steps_annotation_size = 3.5,
                                             steps_y_base = -0.014,
                                          steps_y_height = 0.01,
                                          steps_y_shift = 0.03,
                                          spu_x_shift = 2,
                                             steps_annotation_nudge = 0.1,
                                             steps_values_annotate = T,
                                             steps_names = steps_names,
                                             grid_rows = "nbatch",
                                             grid_cols = "nolap",
                                             y_name = "Empirical standard error",
                                             hline_intercept = 0,
                                             legend_name = "Model",
                                             legend_labels = c("Random MA; Tau:REML; CI:classic", "Random MA; Tau:REML; CI:HK", "Random MA; Tau:REML; CI:HK.SEadhoc", "Random MA; Tau:REML; CI:HK.IQadhoc", "Random MA; Tau:REML; CI:KR"),
                                             post_processing = list(
                                               add_custom_theme = list(
                                                 legend.position="bottom",
                                                 axis.text.x = element_text(angle = -90, 
                                                                            vjust = 0.5, 
                                                                            size = 5) 
                                               )))

EmpSE_MA_shared_reml = nested_loop_plot(resdf = all_contsresults_EmpSE %>%
                                          filter(sharedtime != 1 & effsize == 0) %>%
                                          mutate(M = factor(M)),
                                        grid_labeller = labeller(nbatch = label_value, nolap = label_value),
                                        x = "M", 
                                        x_name = "Number of participants per cluster-period (m)",
                                        methods = c("Emp_SE_MA.random.reml.classic", "Emp_SE_MA.random.reml.HK", "Emp_SE_MA.random.reml.HK.SEadhoc", "Emp_SE_MA.random.reml.HK.IQadhoc", "Emp_SE_MA.random.reml.KR"),
                                           steps = steps,
                                        colors = c(c_maclassic, c_maHK, c_maHK_seadhoc, c_maHK_iqadhoc, c_maKR),
                                        steps_color = "#000000",
                                        steps_annotation_color = "#000000",
                                        point_shapes = c(17, 17, 17, 17, 17),
                                        point_size = 1.2,
                                        steps_annotation_size = 3.5,
                                           steps_y_base = -0.014,
                                        steps_y_height = 0.01,
                                        steps_y_shift = 0.03,
                                        spu_x_shift = 2,
                                           steps_annotation_nudge = 0.1,
                                           steps_values_annotate = T,
                                           steps_names = steps_names,
                                           grid_rows = "nbatch",
                                           grid_cols = "nolap",
                                           y_name = "Empirical standard error",
                                           hline_intercept = 0,
                                           legend_name = "Model",
                                           legend_labels = c("Random MA; Tau:REML; CI:classic", "Random MA; Tau:REML; CI:HK", "Random MA; Tau:REML; CI:HK.SEadhoc", "Random MA; Tau:REML; CI:HK.IQadhoc", "Random MA; Tau:REML; CI:KR"),
                                           post_processing = list(
                                             add_custom_theme = list(
                                               legend.position="bottom",
                                               axis.text.x = element_text(angle = -90, 
                                                                          vjust = 0.5, 
                                                                          size = 5) 
                                             )))
#Saving the results
EmpSE_MA_separate_reml
ggsave("EmpSE_MA_separate_REML.png", height = 13, width = 15)
EmpSE_MA_shared_reml
ggsave("EmpSE_MA_shared_REML.png", height = 13, width = 15)

########################LINEAR-MIXED MODELS vs META-ANALYSIS PLOTS########################

#Prespecifying steps and legend labels for the nested loop plots.
steps = c("K",  "T", "ICC", "CAC", "treateffvar") 
steps_names = c("Clusters (K)", "Periods (T)", "ICC", "CAC", expression(sigma[eta]^2))
legend_labels = c("Model A", "Model B", "Model C", "Model D", "Fixed-effects meta-analysis", "Random MA; Tau:REML; CI:HK")

######Bias######

#Bias for separate period effects (Figure 2)
bias_final_separate = nested_loop_plot(resdf = all_contsresults_bias %>%
                                         filter(sharedtime == 1 & effsize == 0) %>%
                                         mutate(M = factor(M)),
                                       grid_labeller = labeller(nbatch = label_value, nolap = label_value),
                                       x = "M", 
                                       x_name = "Number of participants per cluster-period (m)",
                                       methods = c("Bias_mod_A", "Bias_mod_B", "Bias_mod_C", "Bias_mod_D", "Bias_MA.fixed", "Bias_MA.random.reml.HK"),
                                       steps = steps,
                                       colors = c(c_modA, c_modB, c_modC, c_modD, c_mafixed, c_maHK),
                                       steps_color = "#000000",
                                       steps_annotation_color = "#000000",
                                       point_shapes = c(15, 15, 15, 15, 17, 17),
                                       steps_y_base = -1,
                                       spu_x_shift = 2,
                                       steps_y_height = 0.2,
                                       steps_y_shift = 0.2, #Separate the distance between step
                                       steps_annotation_nudge = 0.1,
                                       steps_annotation_size  = 3,
                                       steps_values_annotate = T,
                                       steps_names = steps_names,
                                       grid_rows = "nbatch",
                                       grid_cols = "nolap",
                                       y_name = "Bias",
                                       hline_intercept = 0,
                                       legend_name = "Model",
                                       legend_labels = legend_labels,
                                       post_processing = list(
                                         add_custom_theme = list(
                                           legend.position="bottom",
                                           axis.text.x = element_text(angle = -90, 
                                                                      vjust = 0.5, 
                                                                      size = 5) 
                                         )))


#Figure 2 but simplified
bias_final_separate_simp = nested_loop_plot(resdf = all_contsresults_bias %>%
                                              filter(sharedtime == 1 & effsize == 0 & nbatch == "5 batches" & nolap == "1 period of overlap" & ICC %in% c(0.01, 0.2) & CAC %in% c(0.5, 1)) %>%
                                         mutate(M = factor(M)),
                                       grid_labeller = labeller(nbatch = label_value, nolap = label_value),
                                       x = "M", 
                                       x_name = "Number of participants per cluster-period (m)",
                                       methods = c("Bias_mod_A", "Bias_mod_B", "Bias_mod_C", "Bias_mod_D", "Bias_MA.fixed", "Bias_MA.random.reml.HK"),
                                       steps = steps,
                                       colors = c(c_modA, c_modB, c_modC, c_modD, c_mafixed, c_maHK),
                                       steps_y_base = -0.6,
                                       spu_x_shift = 1,
                                       steps_y_height = 0.2,
                                       steps_y_shift = 0.2, #Separate the distance between step
                                       steps_annotation_nudge = 0.1,
                                       steps_annotation_size  = 8,
                                       steps_values_annotate = T,
                                       steps_color = "#000000",
                                       steps_annotation_color = "#000000",
                                       point_size = 2.5,
                                       point_shapes = c(15, 15, 15, 15, 17, 17),
                                       steps_names = steps_names,
                                       grid_rows = "nbatch",
                                       grid_cols = "nolap",
                                       y_name = "Bias",
                                       hline_intercept = 0,
                                       legend_name = "Model",
                                       legend_labels = legend_labels,
                                       post_processing = list(
                                         add_custom_theme = list(
                                           legend.position="bottom",
                                           axis.text.x = element_text(angle = -90, 
                                                                      vjust = 0.5, 
                                                                      size = 20),
                                           axis.text.y = element_text(size = 20)
                                         ))) +
  theme(strip.text = element_text(
    size = 20
  ),
  axis.title.x = element_text(size = 30),  # Change x-axis title size
  axis.title.y = element_text(size = 30),  # Change y-axis title size
  legend.position ="right",
  legend.title=element_text(size=15)
  ) +
  guides(color = guide_legend(override.aes = list(size = 15)))


#Bias for shared period effects
bias_final_shared = nested_loop_plot(resdf = all_contsresults_bias %>%
                                       filter(sharedtime != 1 & effsize == 0) %>%
                                       mutate(M = factor(M)),
                                     grid_labeller = labeller(nbatch = label_value, nolap = label_value),
                                     x = "M", 
                                     x_name = "Number of participants per cluster-period (m)",
                                     methods = c("Bias_mod_A", "Bias_mod_B", "Bias_mod_C", "Bias_mod_D", "Bias_MA.fixed", "Bias_MA.random.reml.HK"),
                                     steps = steps,
                                     colors = c(c_modA, c_modB, c_modC, c_modD, c_mafixed, c_maHK),
                                     steps_color = "#000000",
                                     steps_annotation_color = "#000000",
                                     point_size = 1.2,
                                     point_shapes = c(15,15,15,15,17,17,17, 17, 17, 17),
                                     spu_x_shift = 2,
                                     steps_y_base = -0.027,
                                     steps_y_height = 0.005,
                                     steps_y_shift = 0.005, #Separate the distance between step
                                     steps_annotation_nudge = 0.1,
                                     steps_annotation_size  = 3.5,
                                     steps_values_annotate = T,
                                     steps_names = steps_names,
                                     grid_rows = "nbatch",
                                     grid_cols = "nolap",
                                     y_name = "Bias",
                                     hline_intercept = 0,
                                     legend_name = "Model",
                                     legend_labels = legend_labels,
                                     post_processing = list(
                                       add_custom_theme = list(
                                         legend.position="bottom",
                                         axis.text.x = element_text(angle = -90, 
                                                                    vjust = 0.5, 
                                                                    size = 5) 
                                       )))



#Bias for separate period effects without models A and C
bias_final_separate_without = nested_loop_plot(resdf = all_contsresults_bias %>%
                                                 filter(sharedtime == 1 & effsize == 0) %>%
                                                 mutate(M = factor(M)),
                                               grid_labeller = labeller(nbatch = label_value, nolap = label_value),
                                               x = "M", 
                                               x_name = "Number of participants per cluster-period (m)",
                                               methods = c("Bias_mod_B", "Bias_mod_D", "Bias_MA.fixed", "Bias_MA.random.reml.HK"),
                                               steps = steps,
                                               colors = c(c_modB, c_modD, c_mafixed, c_maHK),
                                               point_shapes = c(15, 15, 17, 17),
                                               steps_color = "#000000",
                                               steps_annotation_color = "#000000",
                                               point_size = 1.2,
                                               steps_annotation_size = 3.5,
                                               steps_y_base = -0.019,
                                               steps_y_height = 0.005,
                                               steps_y_shift = 0.005, #Separate the distance between step
                                               steps_annotation_nudge = 0.08,
                                               steps_values_annotate = T,
                                               steps_names = steps_names,
                                               spu_x_shift = 2,
                                               grid_rows = "nbatch",
                                               grid_cols = "nolap",
                                               y_name = "Bias",
                                               hline_intercept = 0,
                                               legend_name = "Model",
                                               legend_labels = c("Model B", "Model D", "Fixed-effects meta-analysis", "Random MA; Tau:REML; CI:HK"),
                                               post_processing = list(
                                                 add_custom_theme = list(
                                                   legend.position="bottom",
                                                   axis.text.x = element_text(angle = -90, 
                                                                              vjust = 0.5, 
                                                                              size = 5) 
                                                 )))

#Saving the results
bias_final_separate
ggsave("bias_final_separate.png", dpi = "retina", height = 13, width = 15)

bias_final_separate_simp
ggsave("bias_final_separate_simp.png", dpi = "retina", height = 13, width = 20)

bias_final_shared
ggsave("bias_final_shared.png", height = 13, width = 15)

bias_final_separate_without
ggsave("bias_final_separate_without_modelA&C.png", height = 13, width = 15)


######Coverage######

#Coverage for separate period effects (Figure 3)
coverage_final_separate = nested_loop_plot(resdf = all_contsresults_coverage %>%
                                             filter(sharedtime == 1 & effsize == 0) %>%
                                             mutate(M = factor(M)),
                                           grid_labeller = labeller(nbatch = label_value, nolap = label_value),
                                           x = "M", 
                                           x_name = "Number of participants per cluster period (m)",
                                           methods = c("Coverage_mod_B", "Coverage_mod_D", "Coverage_MA.fixed", "Coverage_MA.random.reml.HK"),
                                           steps = steps,
                                           colors = c(c_modB, c_modD, c_mafixed, c_maHK),
                                           steps_color = "#000000",
                                           steps_annotation_color = "#000000",
                                           point_size = 1.2,
                                           steps_annotation_size = 3.5,
                                           point_shapes = c(15, 15, 17, 17),
                                           steps_y_base = 0.15,
                                           steps_y_height = 0.08,
                                           steps_y_shift = 0.08,
                                           steps_annotation_nudge = 0.1,
                                           steps_values_annotate = T,
                                           spu_x_shift = 2,
                                           steps_names = steps_names,
                                           grid_rows = "nbatch",
                                           grid_cols = "nolap",
                                           y_name = "Coverage",
                                           hline_intercept = 0.95,
                                           legend_name = "Model",
                                           legend_labels = c("Model B", "Model D", "Fixed-effects meta-analysis", "Random MA; Tau:REML; CI:HK"),
                                           post_processing = list(
                                             add_custom_theme = list(
                                               legend.position="bottom",
                                               axis.text.x = element_text(angle = -90, 
                                                                          vjust = 0.5, 
                                                                          size = 5) 
                                             )))

#Figure 3, but simplified.
coverage_final_separate_simp = nested_loop_plot(resdf = all_contsresults_coverage %>%
                                             filter(sharedtime == 1 & effsize == 0 & nolap == "1 period of overlap" & ICC %in% c(0.01, 0.2) & CAC %in% c(0.5, 1)) %>%
                                             mutate(M = factor(M)),
                                           grid_labeller = labeller(nbatch = label_value, nolap = label_value),
                                           x = "M", 
                                           x_name = "Number of participants per cluster period (m)",
                                           methods = c("Coverage_mod_B", "Coverage_mod_D", "Coverage_MA.fixed", "Coverage_MA.random.reml.HK"),
                                           steps = steps,
                                           colors = c(c_modB, c_modD, c_mafixed, c_maHK),
                                           point_shapes = c(15, 15, 17, 17),
                                           steps_y_base = 0.05,
                                           steps_y_height = 0.08,
                                           steps_y_shift = 0.2,
                                           steps_annotation_nudge = 0.1,
                                           steps_annotation_size  = 7,
                                           steps_values_annotate = T,
                                           steps_color = "#000000",
                                           steps_annotation_color = "#000000",
                                           point_size = 2.5,
                                           spu_x_shift = 1,
                                           steps_names = steps_names,
                                           grid_rows = "nbatch",
                                           grid_cols = "nolap",
                                           y_name = "Coverage",
                                           hline_intercept = 0.95,
                                           legend_name = "Model",
                                           legend_labels = c("Model B", "Model D", "Fixed-effects meta-analysis", "Random MA; Tau:REML; CI:HK"),
                                           post_processing = list(
                                             add_custom_theme = list(
                                               legend.position="bottom",
                                               axis.text.x = element_text(angle = -90, 
                                                                          vjust = 0.5, 
                                                                          size = 20),
                                               axis.text.y = element_text(size = 20)
                                             ))) +
  theme(strip.text = element_text(
    size = 20
  ),
  axis.title.x = element_text(size = 30),  # Change x-axis title size
  axis.title.y = element_text(size = 30),  # Change y-axis title size
  legend.position ="right",
  legend.title=element_text(size=18),
  legend.text = element_text(size = 18)
  ) +
  guides(color = guide_legend(override.aes = list(size = 18)))


#Coverage for shared period effects
coverage_final_shared = nested_loop_plot(resdf = all_contsresults_coverage %>%
                                           filter(sharedtime != 1 & effsize == 0) %>%
                                           mutate(M = factor(M)),
                                         grid_labeller = labeller(nbatch = label_value, nolap = label_value),
                                         x = "M", 
                                         x_name = "Number of participants per cluster period (m)",
                                         methods = c("Coverage_mod_A", "Coverage_mod_B", "Coverage_mod_C", "Coverage_mod_D", "Coverage_MA.fixed", "Coverage_MA.random.reml.HK"),
                                         steps = steps,
                                         colors = c(c_modA, c_modB, c_modC, c_modD, c_mafixed, c_maHK),
                                         steps_color = "#000000",
                                         steps_annotation_color = "#000000",
                                         point_size = 1.2,
                                         steps_annotation_size = 3.5,
                                         point_shapes = c(15, 15, 15, 15, 17, 17),
                                         steps_y_base = 0.15,
                                         steps_y_height = 0.08,
                                         steps_y_shift = 0.08,
                                         steps_annotation_nudge = 0.1,
                                         spu_x_shift = 2,
                                         steps_values_annotate = T,
                                         steps_names = steps_names,
                                         grid_rows = "nbatch",
                                         grid_cols = "nolap",
                                         y_name = "Coverage",
                                         hline_intercept = 0.95,
                                         legend_name = "Model",
                                         legend_labels = legend_labels,
                                         post_processing = list(
                                           add_custom_theme = list(
                                             legend.position="bottom",
                                             axis.text.x = element_text(angle = -90, 
                                                                        vjust = 0.5, 
                                                                        size = 5) 
                                           )))

#Saving the results
coverage_final_separate
ggsave("coverage_final_separate.png", dpi = "retina", height = 13, width = 15)

coverage_final_separate_simp
ggsave("coverage_final_separate_simp.png", dpi = "retina", height = 13, width = 20)

coverage_final_shared
ggsave("coverage_final_shared.png", dpi = "retina", height = 13, width = 15)


######Empirical standard error######

#Empirical standard error for separate period effects
EmpSE_final_separate = nested_loop_plot(resdf = all_contsresults_EmpSE %>%
                                          filter(sharedtime == 1 & effsize == 0) %>%
                                          mutate(M = factor(M)),
                                        grid_labeller = labeller(nbatch = label_value, nolap = label_value),
                                        x = "M", 
                                        x_name = "Number of participants per cluster period (m)",
                                        methods = c("Emp_SE_mod_B",  "Emp_SE_mod_D", "Emp_SE_MA.fixed", "Emp_SE_MA.random.reml.HK"),
                                        steps = steps,
                                        colors = c(c_modB, c_modD, c_mafixed, c_maHK),
                                        point_shapes = c(15, 15, 17, 17),
                                        steps_color = "#000000",
                                        steps_annotation_color = "#000000",
                                        point_size = 1.2,
                                        steps_annotation_size = 3.5,
                                        steps_y_base = -0.03,
                                        steps_y_height = 0.03,
                                        steps_y_shift = 0.03,
                                        steps_annotation_nudge = 0.1,
                                        steps_values_annotate = T,
                                        spu_x_shift = 2,
                                        steps_names = steps_names,
                                        grid_rows = "nbatch",
                                        grid_cols = "nolap",
                                        y_name = "Empirical standard error",
                                        hline_intercept = 0,
                                        legend_name = "Model",
                                        legend_labels = c("Model B", "Model D", "Fixed-effects meta-analysis", "Random MA; Tau:REML; CI:HK"),
                                        post_processing = list(
                                          add_custom_theme = list(
                                            legend.position="bottom",
                                            axis.text.x = element_text(angle = -90, 
                                                                       vjust = 0.5, 
                                                                       size = 5) 
                                          )))

#Empirical standard error for shared period effects
EmpSE_final_shared = nested_loop_plot(resdf = all_contsresults_EmpSE %>%
                                        filter(sharedtime != 1 & effsize == 0) %>%
                                        mutate(M = factor(M)),
                                      grid_labeller = labeller(nbatch = label_value, nolap = label_value),
                                      x = "M", 
                                      x_name = "Number of participants per cluster period (m)",
                                      methods = c("Emp_SE_mod_A", "Emp_SE_mod_B", "Emp_SE_mod_C", "Emp_SE_mod_D", "Emp_SE_MA.fixed", "Emp_SE_MA.random.reml.HK"),
                                      steps = steps,
                                      colors = c(c_modA, c_modB, c_modC, c_modD, c_mafixed, c_maHK),
                                      point_shapes = c(15, 15, 15, 15, 17, 17),
                                      steps_color = "#000000",
                                      steps_annotation_color = "#000000",
                                      point_size = 1.2,
                                      steps_annotation_size = 3.5,
                                      steps_y_base = -0.03,
                                      steps_y_height = 0.03,
                                      steps_y_shift = 0.03,
                                      steps_annotation_nudge = 0.1,
                                      steps_values_annotate = T,
                                      spu_x_shift = 2,
                                      steps_names = steps_names,
                                      grid_rows = "nbatch",
                                      grid_cols = "nolap",
                                      y_name = "Empirical standard error",
                                      hline_intercept = 0,
                                      legend_name = "Model",
                                      legend_labels = legend_labels,
                                      post_processing = list(
                                        add_custom_theme = list(
                                          legend.position="bottom",
                                          axis.text.x = element_text(angle = -90, 
                                                                     vjust = 0.5, 
                                                                     size = 5) 
                                        )))

EmpSE_final_separate
ggsave("EmpSE_final_separate.png", height = 13, width = 15)

EmpSE_final_shared
ggsave("EmpSE_final_shared.png", height = 13, width = 15)

######Rate of non-convergence######

#Rate of non-convergence for separate period effects
nonconv_separate = nested_loop_plot(resdf = all_contsresults_nonconv %>%
                                      filter(sharedtime == 1 & effsize == 0) %>%
                                      mutate(M = factor(M)),
                                    grid_labeller = labeller(nbatch = label_value, nolap = label_value),
                                    x = "M", 
                                    x_name = "Number of participants per cluster period (m)",
                                        methods = c("mod_A", "mod_B", "mod_C", "mod_D", "MA.random.reml.HK", "MA.fixed"),
                                        steps = steps,
                                        colors = c(c_modA, c_modB, c_modC, c_modD, c_maHK, c_mafixed),
                                    point_shapes = c(15, 15, 15, 15, 17, 17),
                                    steps_color = "#000000",
                                    steps_annotation_color = "#000000",
                                    point_size = 1.2,
                                    steps_annotation_size = 3.5,
                                        steps_y_base = -0.05,
                                        steps_y_height = 0.05,
                                        steps_y_shift = 0.05,
                                        steps_annotation_nudge = 0.1,
                                        steps_values_annotate = T,
                                        spu_x_shift = 2,
                                        steps_names = steps_names,
                                        grid_rows = "nbatch",
                                        grid_cols = "nolap",
                                        y_name = "Rate of non-convergence",
                                        hline_intercept = 0,
                                        legend_name = "Model",
                                        legend_labels = c("Model A", "Model B", "Model C", "Model D", "Random MA; Tau:REML; CI:HK", "Fixed-effects meta-analysis"),
                                        post_processing = list(
                                          add_custom_theme = list(
                                            legend.position="bottom",
                                            axis.text.x = element_text(angle = -90, 
                                                                       vjust = 0.5, 
                                                                       size = 5) 
                                          )))

#Rate of non-convergence for shared period effects
nonconv_shared = nested_loop_plot(resdf = all_contsresults_nonconv %>%
                                      filter(sharedtime != 1 & effsize == 0) %>%
                                      mutate(M = factor(M)),
                                    grid_labeller = labeller(nbatch = label_value, nolap = label_value),
                                    x = "M", 
                                    x_name = "Number of participants per cluster period (m)",
                                    methods = c("mod_A", "mod_B", "mod_C", "mod_D", "MA.random.reml.HK", "MA.fixed"),
                                    steps = steps,
                                    colors = c(c_modA, c_modB, c_modC, c_modD, c_maHK, c_mafixed),
                                    point_shapes = c(15, 15, 15, 15, 17, 17),
                                  steps_color = "#000000",
                                  steps_annotation_color = "#000000",
                                  point_size = 1.2,
                                  steps_annotation_size = 3.5,
                                    steps_y_base = -0.05,
                                    steps_y_height = 0.05,
                                    steps_y_shift = 0.05,
                                    steps_annotation_nudge = 0.1,
                                    steps_values_annotate = T,
                                    spu_x_shift = 2,
                                    steps_names = steps_names,
                                    grid_rows = "nbatch",
                                    grid_cols = "nolap",
                                    y_name = "Rate of non-convergence",
                                    hline_intercept = 0,
                                    legend_name = "Model",
                                    legend_labels = c("Model A", "Model B", "Model C", "Model D", "Random MA; Tau:REML; CI:HK", "Fixed-effects meta-analysis"),
                                    post_processing = list(
                                      add_custom_theme = list(
                                        legend.position="bottom",
                                        axis.text.x = element_text(angle = -90, 
                                                                   vjust = 0.5, 
                                                                   size = 5) 
                                      )))

#Saving the results
nonconv_separate
ggsave("nonconv_overall_separate.png", height = 13, width = 15)

nonconv_shared
ggsave("nonconv_overall_shared.png", height = 13, width = 15)


######Meta-analysis check######

#Prespecifying the steps for the nested loop plots.
steps = c("K",  "T", "ICC", "CAC", "treateffvar") 
steps_names = c("Clusters (K)", "Periods (T)", "ICC", "CAC", expression(sigma[eta]^2))

###SJ first

#Separate period effects
MACHECK_SJ_separate = nested_loop_plot(resdf = all_contsresults_MACHECK %>%
                                      filter(sharedtime == 1 & effsize == 0) %>%
                                      mutate(M = factor(M)),
                                    grid_labeller = labeller(nbatch = label_value, nolap = label_value),
                                    x = "M", 
                                    x_name = "Number of participants per cluster period (m)",
                                    methods = c("MA.random.SJ.classic", "MA.random.SJ.HK", "MA.random.SJ.HK.SEadhoc", "MA.random.SJ.HK.IQadhoc", "MA.random.SJ.KR"),
                                    steps = steps,
                                    colors = c(c_maclassic,c_maHK, c_maHK_seadhoc, c_maHK_iqadhoc, c_maKR),
                                    steps_y_base = -0.05,
                                    steps_y_height = 0.05,
                                    steps_y_shift = 0.07,
                                    point_shapes = c(17, 17, 17, 17, 17),
                                    steps_color = "#000000",
                                    steps_annotation_color = "#000000",
                                    point_size = 1.2,
                                    steps_annotation_size = 3.5,
                                    steps_annotation_nudge = 0.1,
                                    steps_values_annotate = T,
                                    spu_x_shift = 2,
                                    steps_names = steps_names,
                                    grid_rows = "nbatch",
                                    grid_cols = "nolap",
                                    y_name = "Percentage of upper and/or lower meta-analysis CIs that exceeded the extremes",
                                    hline_intercept = 0,
                                    legend_name = "Model",
                                    legend_labels = c(c("Random MA; Tau:SJ; CI:classic", "Random MA; Tau:SJ; CI:HK", "Random MA; Tau:SJ; CI:HK.SEadhoc", "Random MA; Tau:SJ; CI:HK.IQadhoc", "Random MA; Tau:SJ; CI:KR")),
                                    post_processing = list(
                                      add_custom_theme = list(
                                        legend.position="bottom",
                                        axis.text.x = element_text(angle = -90, 
                                                                   vjust = 0.5, 
                                                                   size = 5) 
                                      )))


#Shared period effects
MACHECK_SJ_shared = nested_loop_plot(resdf = all_contsresults_MACHECK %>%
                                    filter(sharedtime != 1 & effsize == 0) %>%
                                    mutate(M = factor(M)),
                                  grid_labeller = labeller(nbatch = label_value, nolap = label_value),
                                  x = "M", 
                                  x_name = "Number of participants per cluster period (m)",
                                  methods = c("MA.random.SJ.classic", "MA.random.SJ.HK", "MA.random.SJ.HK.SEadhoc", "MA.random.SJ.HK.IQadhoc", "MA.random.SJ.KR"),
                                  steps = steps,
                                  colors = c(c_maclassic, c_maHK, c_maHK_seadhoc, c_maHK_iqadhoc, c_maKR),
                                  steps_y_base = -0.05,
                                  steps_y_height = 0.05,
                                  steps_y_shift = 0.07,
                                  point_shapes = c(17, 17, 17, 17, 17),
                                  steps_annotation_nudge = 0.1,
                                  steps_color = "#000000",
                                  steps_annotation_color = "#000000",
                                  point_size = 1.2,
                                  steps_annotation_size = 3.5,
                                  steps_values_annotate = T,
                                  spu_x_shift = 2,
                                  steps_names = steps_names,
                                  grid_rows = "nbatch",
                                  grid_cols = "nolap",
                                  y_name = "Percentage of upper and/or lower meta-analysis CIs that exceeded the extremes",
                                  hline_intercept = 0,
                                  legend_name = "Model",
                                  legend_labels = c("Random MA; Tau:SJ; CI:classic", "Random MA; Tau:SJ; CI:HK", "Random MA; Tau:SJ; CI:HK.SEadhoc", "Random MA; Tau:SJ; CI:HK.IQadhoc", "Random MA; Tau:SJ; CI:KR"),
                                  post_processing = list(
                                    add_custom_theme = list(
                                      legend.position="bottom",
                                      axis.text.x = element_text(angle = -90, 
                                                                 vjust = 0.5, 
                                                                 size = 5) 
                                    )))

###Then REML

#Separate period effects
MACHECK_reml_separate = nested_loop_plot(resdf = all_contsresults_MACHECK %>%
                                         filter(sharedtime == 1 & effsize == 0) %>%
                                         mutate(M = factor(M)),
                                       grid_labeller = labeller(nbatch = label_value, nolap = label_value),
                                       x = "M", 
                                       x_name = "Number of participants per cluster period (m)",
                                       methods = c("MA.random.reml.classic", "MA.random.reml.HK", "MA.random.reml.HK.SEadhoc", "MA.random.reml.HK.IQadhoc", "MA.random.reml.KR"),
                                       steps = steps,
                                       colors = c(c_maclassic, c_maHK, c_maHK_seadhoc, c_maHK_iqadhoc, c_maKR),
                                       steps_y_base = -0.05,
                                       steps_y_height = 0.05,
                                       steps_y_shift = 0.07,
                                       point_shapes = c(17, 17, 17, 17, 17),
                                       steps_color = "#000000",
                                       steps_annotation_color = "#000000",
                                       point_size = 1.2,
                                       steps_annotation_size = 3.5,
                                       steps_annotation_nudge = 0.1,
                                       steps_values_annotate = T,
                                       spu_x_shift = 2,
                                       steps_names = steps_names,
                                       grid_rows = "nbatch",
                                       grid_cols = "nolap",
                                       y_name = "Percentage of upper and/or lower meta-analysis CIs that exceeded the extremes",
                                       hline_intercept = 0,
                                       legend_name = "Model",
                                       legend_labels = c("Random MA; Tau:REML; CI:classic", "Random MA; Tau:REML; CI:HK", "Random MA; Tau:REML; CI:HK.SEadhoc", "Random MA; Tau:REML; CI:HK.IQadhoc", "Random MA; Tau:REML; CI:KR"),
                                       post_processing = list(
                                         add_custom_theme = list(
                                           legend.position="bottom",
                                           axis.text.x = element_text(angle = -90, 
                                                                      vjust = 0.5, 
                                                                      size = 5) 
                                         )))


#Shared period effects
MACHECK_reml_shared = nested_loop_plot(resdf = all_contsresults_MACHECK %>%
                                       filter(sharedtime != 1 & effsize == 0) %>%
                                       mutate(M = factor(M)),
                                     grid_labeller = labeller(nbatch = label_value, nolap = label_value),
                                     x = "M", 
                                     x_name = "Number of participants per cluster period (m)",
                                     methods = c("MA.random.reml.classic", "MA.random.reml.HK", "MA.random.reml.HK.SEadhoc", "MA.random.reml.HK.IQadhoc", "MA.random.reml.KR"),
                                     steps = steps,
                                     colors = c(c_maclassic, c_maHK, c_maHK_seadhoc, c_maHK_iqadhoc, c_maKR),
                                     steps_y_base = -0.05,
                                     steps_y_height = 0.05,
                                     steps_y_shift = 0.07,
                                     point_shapes = c(17, 17, 17, 17, 17),
                                     steps_color = "#000000",
                                     steps_annotation_color = "#000000",
                                     point_size = 1.2,
                                     steps_annotation_size = 3.5,
                                     steps_annotation_nudge = 0.1,
                                     steps_values_annotate = T,
                                     spu_x_shift = 2,
                                     steps_names = steps_names,
                                     grid_rows = "nbatch",
                                     grid_cols = "nolap",
                                     y_name = "Percentage of upper and/or lower meta-analysis CIs that exceeded the extremes",
                                     hline_intercept = 0,
                                     legend_name = "Model",
                                     legend_labels = c("Random MA; Tau:REML; CI:classic", "Random MA; Tau:REML; CI:HK", "Random MA; Tau:REML; CI:HK.SEadhoc", "Random MA; Tau:REML; CI:HK.IQadhoc", "Random MA; Tau:REML; CI:KR"),
                                     post_processing = list(
                                       add_custom_theme = list(
                                         legend.position="bottom",
                                         axis.text.x = element_text(angle = -90, 
                                                                    vjust = 0.5, 
                                                                    size = 5) 
                                       )))


#Saving the results
MACHECK_SJ_separate
ggsave("MACHECK_SJ_separate.png", height = 13, width = 15)

MACHECK_SJ_shared
ggsave("MACHECK_SJ_shared.png", height = 13, width = 15)

MACHECK_reml_separate
ggsave("MACHECK_reml_separate.png", height = 13, width = 15)

MACHECK_reml_shared
ggsave("MACHECK_reml_shared.png", height = 13, width = 15)





